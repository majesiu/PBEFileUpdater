...
with open('pbe_rosters.csv', newline='', encoding="UTF-8") as ootpcsvfile:
    ootpdump = csv.reader(ootpcsvfile, delimiter=',', quotechar='"')
    counter = 0
    for row in ootpdump:
        if(not row[0].startswith("//")):
            key = row[6].strip()+" "+row[5].strip()
            players[key] = row
            counter += 1
    print("Loaded", counter, "players from pbe_rosters6.csv")

    
with open('milpbe_rosters.csv', newline='', encoding="UTF-8") as ootpcsvfile:
    ootpdump = csv.reader(ootpcsvfile, delimiter=',', quotechar='"')
    counter = 0
    for row in ootpdump:
        if(not row[0].startswith("//")):
